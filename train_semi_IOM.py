...
def visual_multi_mask(save_dir,mask_list):
    ...
    for i in range(n):
        if type(mask_list[i])==torch.Tensor:
            plt.imshow(mask_list[i].cpu().numpy())
        else:
    ...
    for data in metric_logger.log_semi(loader,print_freq,length,header):
        ...
        for b in range(args.batch_size):
            ...
            for t in range(sam_mask.shape[0]):
                ...
                inter = (_foreground * c[b]).sum()/(_foreground+c[b]-_foreground * c[b]).sum()
                if inter > 0.5:
                    if inter > inter_top1:
                        inter_top1 = inter
            ...
            if cnt ==sam_mask.shape[0]:
                ...
                refine_tea_pred[b][foreground.bool()] = 1
...
def main(args):
    ...
    for epoch in range(max(0, resume_epoch+1), args.epochs):
        ...
        if dist.is_initialized():
            dist.barrier()
        dist.all_reduce(patience,op=dist.ReduceOp.MIN)
        patience = torch.tensor(0).cuda() # early stop signal
        if patience.item()==5:
