...
from dataset_semi  import ReferDataset_Semi
import torchvision
...
os.environ['CUBLAS_WORKSPACE_CONFIG'] = ':4096:8'  # 设置为指定的值
run = wandb.init(project="lavt")
config = run.config
...
def train_one_epoch(model, criterion, optimizer, data_loader, lr_scheduler, epoch, print_freq,
    ...
    for data in metric_logger.log_every(data_loader, print_freq, header):
        ...
        # plt.imsave('vis/pred.jpg',output[0].argmax(0).cpu().numpy())
        # plt.imsave('vis/target.jpg',target[0].cpu().numpy())
        ...
        run.log({"loss": loss,'lr':optimizer.param_groups[0]["lr"]})
...
def main(args):
    ...
    model = segmentation.__dict__[args.model](pretrained=args.pretrained_swin_weights,
                                              args=args)
    model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model)  #将模型中的BatchNorm层替换为同步批归一化层。通过这个转换，可以确保在多个GPU上训练时的一致性，并提高模型的性能和效果。
    model.cuda()    #将模型及其参数从CPU内存中复制到GPU内存中，以便在GPU上执行后续的计算操作。这样做可以充分利用GPU的并行计算能力，加速深度学习任务的执行
    ...
    for epoch in range(max(0, resume_epoch+1), args.epochs):
        ...
        #iou, overallIoU = evaluate_from_test(model, data_loader_test, bert_model,next(model.parameters()).device)
